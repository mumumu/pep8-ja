...
この PEP は、``if`` 文に含まれるネストされた式の部分と、継続行のはじめの部分を区別するかどうか(またはどうやって区別するか)については立場を示しません。2文字のキーワードの後にスペースを一つ置き、開き括弧を書くと、if 文の中に置く最初の式は、当然4つのスペースを空けてインデントしなければなりません。許容できるやり方はいくつかあります::
