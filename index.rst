...
行を継続する場合は、折り返された要素を縦に揃えるようにすべきです。括弧やブラケットおよび波括弧で囲まれた要素については、Python が暗黙のうちに行を結合することを利用して揃えます。そうでない場合は、手でインデントさせることで揃えます。突き出しインデント(訳注: はじめの行以外をインデントし、はじめの行を目立たせるやり方) を使う場合は、次のことを考慮すべきです： はじめの行には引数を付けずに次の行以降をインデントし、継続行だとはっきりわかるようにしましょう。
...
モジュール名はファイル名にマップされますが、大文字小文字を区別して長い名前を切り捨てるファイルシステムもあるため、モジュールの名前はかなり短いものを選ぶことが重要です - この問題はUnixでは起きませんが、コードを古い Mac や Windows や DOS で動かすと問題が起こるかもしれません。
...
- 拡張比較(rich comparion)を使って並び替えを実装する場合、特定の比較を実行するだけの他のコードに依存するよりはむしろ、全ての演算 (``__eq__``, ``__ne__``, ``__lt__``, ``__le__``, ``__gt__``, ``__ge__``) をすべて実装するのがベストです。
...
- ラムダ式を直接名前に結びつける代入文を書くのではなくて、常に def 文を使いましょう。
