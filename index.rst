...
- この PEP では、以前アノテーションの使い方について実験することを勧めていましたが、もはや推奨されまません。

- しかし、標準ライブラリ以外では、PEP 484 のルールの範囲内で実験することが推奨されています。たとえば、PEP 484 の型アノテーションの書式を使った大規模なサードパーティのライブラリやアプリケーションを作り、これらのアノテーションを追加するのがどれだけ簡単だったかをレビューしたり、アノテーションがない場合にコードの理解度があがるかどうかを観察すること、などです。
...
mypy 型チェッカー [6]_ が今のところこの記法をサポートしていますし、他の型チェッカーもこれに追随することが推奨されます。
